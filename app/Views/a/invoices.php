        ...
        foreach ($filtered as $inv): ?>
          <tr>
            ...
            <td class="actions">
              ...
              <a href="generate_invoice.php?invoice=<?= $inv[0] ?>" class="btn-download" title="Download"><i class="fa fa-download"></i></a>
...
.filter-btn:hover { background: #0e9a75; }
...
.status { padding: 4px 10px; border-radius: 20px; font-size: 0.85rem; font-weight: 500; }
...
.actions a { margin-right: 8px; color: #555; font-size: 1rem; transition: 0.2s; }
.actions a:hover { color: #10b981; transform: scale(1.1); }
...
document.querySelectorAll('.btn-email').forEach(btn => {
  btn.addEventListener('click', e => {
    ...
    alert(`✅ Invoice ${id} has been sent to ${email}`);
  });
});
